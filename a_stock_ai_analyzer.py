...
def call_deepseek(config: Config, structured_text: str) -> str:
    ...
    system_prompt = (
        "你是严谨的A股研究助理。请仅基于用户提供的数据生成分析报告，"
        "不得编造未提供的数据，不得使用空泛表达；每个结论后必须附“依据：...”字段，"
        "且依据必须引用输入数据中的具体指标、数值、趋势或时间范围。"
        "不得给出绝对买卖指令，策略建议必须是条件化表述；"
        "必须明确提示：仅供参考，不构成投资建议。"
    )
    user_prompt = f"""
    ...
    输出要求：
    - 使用中文，结构清晰，避免夸大收益。
    - 严禁编造输入中没有的数据；若关键数据缺失，请明确写“暂无数据”，并说明这会如何限制判断。
    - 每个判断性结论后都必须附“依据：...”字段，依据需引用下方输入数据中的具体指标、数值、趋势或时间范围，避免空泛表达。
    - 不得给出“立即买入/卖出”等绝对买卖指令；短中长期策略必须使用条件化建议。

    请严格按以下结构输出：
    1. 一句话结论：说明当前更偏进攻、防守、观望还是回避。
       依据：引用最关键的价格、估值、财务或技术面数据。
    2. 核心投资逻辑：最多 3 条，每条必须引用输入数据。
       - 逻辑 1：...
         依据：...
       - 逻辑 2：...
         依据：...
       - 逻辑 3：...
         依据：...
    3. 主要矛盾：公司当前最关键的问题是什么。
       依据：...
    4. 财务质量判断：分别评价趋势、盈利能力、现金流、负债。
       依据：...
    5. 估值判断：评价历史分位、行业分位、盈利质量是否匹配估值。
       依据：...
    6. 技术面交易框架：给出支撑位、压力位、触发条件、失效条件。
       依据：...
    7. 短中长期策略：分别给出短期（3-5个交易日）、中期（2-3个月）、长期（1年左右）的条件化建议，不给绝对买卖指令。
       依据：...
    8. 反方观点：说明什么情况出现代表原判断可能错误。
       依据：...
    9. 风险清单：按高/中/低影响排序。
       依据：...
    10. 跟踪指标：列出未来需要重点观察的 3-5 个数据。
        依据：...
